#OU Analysis of extremophile fish
...
total_process <- function (dat_obj){
  ...
  fit_name <- paste0("fishes/arbutus/OU/Fits/fit_", num)
  saveRDS(fit, file = fit_name)
  result <- run_arb(fit)
  rds_name <- paste0("fishes/arbutus/OU/pvals/pvals_", num)
  ...
  pval_name <- paste0("fishes/arbutus/OU/plots/arbutus_", num, ".png")
  ggsave(pval_name)
}
